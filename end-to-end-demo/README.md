...
### System Requirement
- *httpie* - install from https://httpie.io/cli:
  - for Ubuntu: `sudo apt-get install httpie`
- *git*
- *git config* - ONLY ON WINDOWS: to be able to successfully build on Windows, please run the following commands to enforece the LF line endings:
  - ```
    git config core.autocrlf false
    git config core.eol lf
    ```
  - if you already cloned the repo, then please run the following to "fix" the line endings:
    ```
...
- Open the Kafka UI (it has to be configured the first time): http://localhost:8090

## Prepare WildFly

- `docker compose up --build -d hiking-manager`
- Go to http://localhost:8082/hikr-1.0-SNAPSHOT/hikes.html
...
- `cat register-hiking-connector.json | http POST http://localhost:8083/connectors/`
  - `curl -X POST -H "Accept:application/json" -H "Content-Type:application/json" http://localhost:8083/connectors/ -d @register-hiking-connector.json`
- `http localhost:8083/connectors/hiking-connector/status`
  - `curl -i http://localhost:8083/connectors/hiking-connector/status`
...
- `http http://localhost:8081/subjects/dbserver1_inventory_Hike-value/versions/1 | jq '.schema | fromjson'`
  - `curl http://localhost:8081/subjects/dbserver1_inventory_Hike-value/versions/1 | jq '.schema | fromjson'`
...
- `cat jdbc-sink.json | http POST http://localhost:8083/connectors/`
  - `curl -X POST -H "Accept:application/json" -H "Content-Type:application/json" http://localhost:8083/connectors/ -d @jdbc-sink.json`
...
- `cat register-hiking-connector-json.json | http POST http://localhost:8083/connectors/`
  - `curl -X POST -H "Accept:application/json" -H "Content-Type:application/json" http://localhost:8083/connectors/ -d @register-hiking-connector-json.json`
